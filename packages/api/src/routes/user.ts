...
import { UserTable, type User, UserKeyTable, SessionTable } from '../db/schema'
...
export function sanitizeUserIdInput<K extends keyof T, T>({
  ...
  if (input?.[idField] && ctx.session?.user?.userId !== input[idField]) {
    throw new TRPCError({
      code: 'UNAUTHORIZED',
...
async function getUser({
  ...
  const userKeys = ctx.db.select().from(UserKeyTable).where(eq(UserKeyTable.userId, userId))
...
const signIn = async ({
  ctx,
  input,
}: { ctx: ApiContextProps; input: Input<typeof SignInSchema> }) => {
  ...
  try {
    if (input.provider && input.token) {
      ...
      const user = await getUserFromProviderUserAuth(
        'apple',
        providerUserAuth(ctx.auth, 'apple', payload.sub)
      )
